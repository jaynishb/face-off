...
## The design box from project.godot. Only the shell screens need it: because
## stretch/aspect is "expand", a phone taller than 16:9 gives the real viewport
## more height than this, and a content block laid out against the design box
## alone would sit pinned to the top with dead space beneath it.
const NOMINAL_HEIGHT := 1280.0

## How far down to push a shell screen's primary content block so it sits
## centred in the REAL visible height rather than in the design box. Zero when
## the viewport matches the design box, so it is never a visual change there.
##
## Corner chrome — a screen's own back/settings button, a modal's close button —
## stays pinned to the true screen corner and must NOT use this. Only the
## primary content block does.
##
## Gameplay must not use it either: a game's geometry comes from play_rect() and
## the player rects, which already account for the real viewport.
func shell_top_offset() -> float:
	return maxf(0.0, (height() - NOMINAL_HEIGHT) * 0.5)
