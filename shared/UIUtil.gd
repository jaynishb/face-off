...
## Returns the Tween so a caller that repositions the node later can kill and
## restart it. The base y is captured HERE, once: start a float before the
## node has been laid out and it will animate around the wrong origin forever,
## which is what put the menu mascots on top of the title.
static func idle_float(node: Control, amplitude: float = 8.0, duration: float = 1.4, delay: float = 0.0) -> Tween:
